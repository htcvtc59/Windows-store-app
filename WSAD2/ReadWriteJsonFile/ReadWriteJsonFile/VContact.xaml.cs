using ReadWriteJsonFile.Common;
using System;
...
namespace ReadWriteJsonFile
{
    ...
    /// </summary>
    public sealed partial class VContact : Page
    {
        ...
        private async Task deserializeJsonAsync()
        {
            ...
            myContact = (List<Contact>)jsonSerializer.ReadObject(myStream);

            foreach (var contact in myContact)
            {
                content += String.Format("Name: {0}, Phone: {1}, Group: {2} ... ", contact.Name, contact.Phone, contact.Group);
            }
